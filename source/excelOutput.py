	...
	def write_to_meta_sheet(self,data,sheet):
		...
		for iteration in data:
			...
			for key in static_value._output_field:
				...
				#write value
				if key == 'teamId':
					team_name=static_value.dic[data[row].get(key)]
					sheet.write(row+1,column,team_name)
					"""v1.1 begin added Sub-domain,Tribe field by Calvin"""
				elif key == 'Sub-domain':
					sheet.write(row+1,column,static_value.dic[data[row].get('teamId') + 'Sub-domain'])
				elif key == 'Tribe':
					sheet.write(row+1,column,static_value.dic[data[row].get('teamId') + 'Tribe'])
					"""v1.1 end"""
				else:
